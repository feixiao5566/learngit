...
ros::Publisher *pub_vt = NULL;
void image_callback(const uhf_rfid_api::UhfRfid::ConstPtr& image)
{
...
//  lv->on_image(&image->data[0], (image->encoding == "rgb8" ? false : true), image->width, image->height);
  ...
  vt_output.current_id = 0;//lv->get_current_vt();
  vt_output.relative_rad = 0;//lv->get_relative_rad();
...
//  image_transport::ImageTransport it(node);
  ros::Subscriber sub = node.subscribe<uhf_rfid_api::UhfRfid>("messageepc", 0, image_callback);
