...
void rfid_callback( uhf_rfid_api::UhfRfid rfid)
{
  ...
  static ratslam_ros::ViewTemplate vt_output;//消息类型

  // FIXME: intelligent handling of image encoding
//on_image把data传过来了
  lv->on_image((const unsigned char *)rfid.data.c_str());//, (image->encoding == "rgb8" ? false : true), image->width, image->height);

  vt_output.header.stamp = ros::Time::now();
  vt_output.header.seq++;
  vt_output.current_id = lv->get_current_vt();//以后要get->current_vt();
//  vt_output.data = rfid.data;//data不能在这，data应该在on_image处理
//  vt_output.current_id = lv->get_current_vt();
//  vt_output.relative_rad = lv->get_relative_rad();
